...
import { BookOpen, Download, Eye, ArrowRight, Sparkles, TrendingUp, FileText } from 'lucide-react';
...
export default function NotesPage() {
  ...
  useEffect(() => {
    setLocalFilters({ type: 'notes' });
    setFilters({ type: 'notes' });
  }, []);

  const handleFilterChange = (newFilters: ContentFilters) => {
    const mergedFilters = { ...newFilters, type: 'notes' };
  ...
  const notesContents = contents.filter(c => c.type === 'notes');
        ...
        >
          <div className="mb-6 sm:mb-8">
            <div className="flex items-center gap-2 mb-2">
              <div className="h-9 w-9 sm:h-10 sm:w-10 rounded-lg sm:rounded-xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-lg shadow-blue-500/25">
            ...
            <p className="text-zinc-500 dark:text-zinc-400 text-sm sm:text-base">
              Access comprehensive study materials for all semesters
            </p>
          </div>
          ...
          {isLoading ? (
                ...
                <Card key={i} className="animate-pulse overflow-hidden">
                  <div className="p-3 sm:p-5 space-y-3 sm:space-y-4">
                    <div className="flex gap-2">
                      <div className="h-5 w-12 sm:h-6 sm:w-16 rounded-lg bg-zinc-200 dark:bg-zinc-800" />
                      <div className="h-5 w-10 sm:h-6 sm:w-12 rounded-lg bg-zinc-200 dark:bg-zinc-800" />
                    </div>
                    ...
                    <div className="flex gap-2">
                      <div className="h-4 sm:h-5 w-12 sm:h-5 sm:w-14 rounded bg-zinc-200 dark:bg-zinc-800" />
            ...
            <Card className="py-12 sm:py-16">
              <div className="flex flex-col items-center justify-center text-center px-4">
                ...
                <p className="text-zinc-500 dark:text-zinc-400 max-w-sm text-sm sm:text-base">
                  Study materials will appear here once uploaded by teachers
                </p>
              </div>
            </Card>
          ) : (
            <>
              <div className="grid gap-3 sm:gap-4 grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-3">
                {notesContents.map((content, index) => (
                  <motion.div
                    key={content._id}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
